use math_explorer::pure_math::differential_geometry::surface::{ParametricSurface, Sphere, Torus};
use math_explorer::pure_math::differential_geometry::operators::laplace_beltrami;
use math_explorer::pure_math::analysis::pde::{SecondOrderLinearPde2D, PdeClassification};
use math_explorer::pure_math::differential_geometry::heat_equation::HeatEquationSolver;
use math_explorer::pure_math::differential_geometry::mean_curvature_flow::{DiscreteSurface, MeanCurvatureFlow};
use nalgebra::Point3;
...
fn test_sphere_properties() {
    ...
    assert!((k - expected_k).abs() < 1e-4, "Gaussian curvature mismatch: {} vs {}", k, expected_k);
    assert!((h.abs() - expected_h).abs() < 1e-4, "Mean curvature mismatch: {} vs {}", h, expected_h);
}
...
fn test_torus_curvature_sign() {
    let major = 3.0;
    let minor = 1.0;
    let torus = Torus { major_radius: major, minor_radius: minor };
...
fn test_pde_classification() {
    let laplace = SecondOrderLinearPde2D { a: 1.0, b: 0.0, c: 1.0 };
    assert_eq!(laplace.classify(), PdeClassification::Elliptic);

    let wave = SecondOrderLinearPde2D { a: 1.0, b: 0.0, c: -1.0 };
...
fn test_heat_equation_smoothing() {
    ...
    // Measure max temp
    let max_temp_init = solver.u_grid.iter().flatten().fold(0.0f64, |a, &b| a.max(b));
    ...
    let max_temp_final = solver.u_grid.iter().flatten().fold(0.0f64, |a, &b| a.max(b));

    // Heat should spread, lowering the max temp
    assert!(max_temp_final < max_temp_init, "Max temp should decrease: {} -> {}", max_temp_init, max_temp_final);
}
...
fn test_mean_curvature_flow_shrinkage() {
    ...
    let mut flow = MeanCurvatureFlow { surface: discrete_sphere };
    ...
    assert!(r_final < r_init, "Sphere should shrink under MCF: {} -> {}", r_init, r_final);
}
