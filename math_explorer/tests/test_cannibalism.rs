...
mod tests {
    ...
    #[allow(deprecated)]
    use math_explorer::applied::cannibalism::two_dimensional_ode::{dndt, dcdt};
    ...
    fn test_struct_implementation_matches_legacy() {
        ...
        assert!((derivative[0] - legacy_dn).abs() < 1e-10, "Model derivative dN/dt should match legacy");
        assert!((derivative[1] - legacy_dc).abs() < 1e-10, "Model derivative dC/dt should match legacy");
    }
}
