use math_explorer::applied::win_ratio::pair_comparison::{
    WinRatioAnalysis, HigherIsBetter, LowerIsBetter, ThresholdComparator,
    ComparisonResult, unmatched_pairs,
};
...
fn test_mixed_strategies() {
    ...
    assert_eq!(analysis.compare_subjects(&subj_a, &subj_b), ComparisonResult::Win);
    ...
    assert_eq!(analysis.compare_subjects(&subj_a, &subj_c), ComparisonResult::Win);
}
...
fn test_threshold_strategy() {
    ...
    let analysis = WinRatioAnalysis::new()
        .add_strategy(Box::new(ThresholdComparator::new(5.0)));

    let subj_a = vec![100.0];
    let subj_b = vec![104.0]; // Diff is 4.0 -> Tie
    let subj_c = vec![106.0]; // Diff is 6.0 -> Win for C (106 > 100)

    assert_eq!(analysis.compare_subjects(&subj_a, &subj_b), ComparisonResult::Tie);
    assert_eq!(analysis.compare_subjects(&subj_b, &subj_a), ComparisonResult::Tie);

    assert_eq!(analysis.compare_subjects(&subj_c, &subj_a), ComparisonResult::Win);
    assert_eq!(analysis.compare_subjects(&subj_a, &subj_c), ComparisonResult::Loss);
}
