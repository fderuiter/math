...
mod tests {
    ...
    fn test_legacy_functions() {
        ...
        let cap = capacity(n, d).unwrap();
        assert!((cap - 0.7).abs() < 1e-6);

        let cycles = cycles_to_capacity(0.7, d).unwrap();
    ...
    fn test_strong_types() {
        ...
        let cycles = model.n70(dod).unwrap();
        assert!((cycles.as_f64() - 600.0).abs() < 50.0);

        let cap = model.capacity(cycles, dod).unwrap();
        assert!((cap.as_f64() - 0.7).abs() < 1e-6);

        let calculated_cycles = model.cycles_to_capacity(Capacity::new(0.7).unwrap(), dod).unwrap();
    ...
    fn test_custom_model() {
        ...
        let cycles_imp = improved.n70(dod).unwrap().as_f64();
