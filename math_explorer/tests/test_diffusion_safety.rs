...
fn test_finite_difference_apply_step_correctness() {
    ...
    let reaction = |_u: f64, _v: f64| (-_u * 0.5, 0.0);
    ...
    assert!(
        (out_u[idx] - expected_next_u).abs() < 1e-10,
        "Interior point calculation mismatch"
    );
    ...
    assert!(
        (out_u[idx] - expected_next_u).abs() < 1e-10,
        "Left boundary calculation mismatch"
    );
    ...
    assert!(
        (out_u[idx] - expected_next_u).abs() < 1e-10,
        "Right boundary calculation mismatch"
    );
}
