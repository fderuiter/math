...
fn main() {
    ...
    // Parameters matching bench_morphogenesis.rs (as closely as possible)
    // TuringSystem::new(size, 0.1, 0.05, 1.0) -> d_u=0.1, d_v=0.05, dx=1.0
    let d_u = 0.1;
    let d_v = 0.05;
    let dx = 1.0;
    ...
    // Initialize with some pattern to avoid optimizing away zeros (though unlikely)
    for i in 0..size {
