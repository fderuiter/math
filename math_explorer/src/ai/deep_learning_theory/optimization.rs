...
pub struct Adam {
    ...
    beta2: f64,
    epsilon: f64,
    state_matrix: HashMap<(usize, ParamType), AdamState<Matrix>>,
    state_vector: HashMap<(usize, ParamType), AdamState<Vector>>,
}
...
impl Optimizer for Adam {
    fn update_matrix(
        ...
        let state = self.state_matrix.entry(key).or_insert_with(|| AdamState {
        ...
        state.v = self.beta2 * &state.v + (1.0 - self.beta2) * grad_sq;
    ...
    fn update_vector(
        ...
        let key = (layer_idx, param_type);
        let state = self.state_vector.entry(key).or_insert_with(|| AdamState {
