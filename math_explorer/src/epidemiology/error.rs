...
pub enum EpidemiologyError {
    ...
    DimensionMismatch { f_rows: usize, f_cols: usize, v_rows: usize, v_cols: usize },
}

impl fmt::Display for EpidemiologyError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::SingularTransitionMatrix => write!(f, "Transition matrix V is singular, Next Generation Matrix cannot be computed."),
            Self::InvalidParameter { name, value } => write!(f, "Invalid parameter {}: {}", name, value),
            Self::DimensionMismatch { f_rows, f_cols, v_rows, v_cols } => write!(f, "Matrix dimensions mismatch: F=({}, {}), V=({}, {})", f_rows, f_cols, v_rows, v_cols),
        }
    }
}
