...
/// # Returns
///
/// The mean squared error or a ClimateError if shapes mismatch.
pub fn mse_loss(y_true: &DMatrix<f32>, y_pred: &DMatrix<f32>) -> Result<f32, ClimateError> {
    if y_true.shape() != y_pred.shape() {
        return Err(ClimateError::ShapeMismatch {
            ...
            actual: y_pred.shape(),
            message: "Matrices must have the same shape for MSE.".to_string(),
        });
    }
...
/// # Returns
///
/// The average Earth Mover's Distance or a ClimateError if shapes mismatch.
pub fn earth_movers_distance(z1: &DMatrix<f32>, z2: &DMatrix<f32>) -> Result<f32, ClimateError> {
    if z1.shape() != z2.shape() {
        return Err(ClimateError::ShapeMismatch {
            expected: z1.shape(),
            actual: z2.shape(),
            message: "Matrices must have the same shape for EMD.".to_string(),
        });
    }
    if z1.ncols() == 0 {
        return Ok(0.0);
    ...
    for i in 0..z1.ncols() {
        ...
        // Use total_cmp for safe sorting (handles NaNs, though NaNs are bad in loss)
