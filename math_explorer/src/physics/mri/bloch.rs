...
/// Represents the physical properties of a tissue.
#[derive(Clone, Copy, Debug)]
pub struct Tissue {
    /// Equilibrium magnetization $M_0$.
    pub m0: f64,
    /// Longitudinal relaxation time $T_1$ (seconds).
    pub t1: f64,
    /// Transverse relaxation time $T_2$ (seconds).
    pub t2: f64,
}

impl Tissue {
    /// Creates a new Tissue definition.
    pub fn new(m0: f64, t1: f64, t2: f64) -> Self {
        Self { m0, t1, t2 }
    }
}

/// Internal helper struct to define the Bloch equations as an OdeSystem.
struct BlochSystem<'a> {
    tissue: &'a Tissue,
    b_field: Vector3<f64>,
}

impl<'a> OdeSystem<Vector3<f64>> for BlochSystem<'a> {
    fn derivative(&self, _t: f64, state: &Vector3<f64>) -> Vector3<f64> {
        ...
        let transverse_decay = Vector3::new(
            state.x / self.tissue.t2,
            state.y / self.tissue.t2,
            0.0
        );
        ...
        let longitudinal_recovery = Vector3::new(
            0.0,
            0.0,
            (state.z - self.tissue.m0) / self.tissue.t1
        );
...
pub struct BlochSimulator {
    ...
    /// The tissue properties being simulated.
    /// Note: Storing tissue here is debatable if we want to simulate different tissues,
    /// but usually a simulator instance tracks one voxel.
    pub tissue: Tissue,
}
...
impl BlochSimulator {
    ...
    /// * `tissue` - Physical properties of the tissue.
    pub fn new(initial_magnetization: Vector3<f64>, tissue: Tissue) -> Self {
        Self {
    ...
    pub fn step_with<S>(&mut self, dt: f64, b_field: Vector3<f64>, solver: &S)
    where
    ...
    /// * `dt` - Time step in seconds.
    /// * `b_field` - Magnetic field vector $\vec{B}$ in Tesla.
    pub fn step(&mut self, dt: f64, b_field: Vector3<f64>) {
        self.step_with(dt, b_field, &Euler)
    }
}
...
mod tests {
    ...
    fn test_bloch_relaxation() {
        ...
        let tissue = Tissue::new(m0, t1, t2);
        let mut bloch = BlochSimulator::new(initial_m, tissue);
    ...
    fn test_bloch_rk4_accuracy() {
        ...
        for _ in 0..steps {
            bloch.step_with(dt, b_field, &RungeKutta4);
        }
