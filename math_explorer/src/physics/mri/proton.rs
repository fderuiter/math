...
/// Error type for Quantum MRI calculations.
#[derive(Debug, Clone, PartialEq)]
pub enum MriError {
    /// The temperature provided was <= 0 Kelvin.
...
impl fmt::Display for MriError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            MriError::InvalidTemperature(t) => write!(f, "Temperature must be positive, got {} K", t),
        }
    }
}
...
pub fn boltzmann_ratio(temperature: f64, b0: f64) -> Result<f64, MriError> {
    if temperature <= 0.0 {
        return Err(MriError::InvalidTemperature(temperature));
    }
