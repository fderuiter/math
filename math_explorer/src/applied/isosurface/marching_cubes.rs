...
pub fn extract_isosurface(grid: &VoxelGrid, threshold: f32) -> Result<Mesh, IsosurfaceError> {
    ...
    for z in 0..grid.depth - 1 {
        ...
        for y in 0..grid.height - 1 {
            ...
            // Preload "Left Face" values for x=0.
            // These correspond to vertices 0, 3, 4, 7.
            // In the loop, we will only load the "Right Face" (1, 2, 5, 6) and then
            // shift them to become the Left Face for the next iteration (x+1).
            let base_idx_iter = zy_base;
            let mut v0 = unsafe { *data.get_unchecked(base_idx_iter) };
            let mut v3 = unsafe { *data.get_unchecked(base_idx_iter + stride_y) };
            let mut v4 = unsafe { *data.get_unchecked(base_idx_iter + stride_z) };
            let mut v7 = unsafe { *data.get_unchecked(base_idx_iter + stride_y + stride_z) };

            // Precompute mask for Left Face
            let mut mask_l = 0;
            if v0 < threshold { mask_l |= 1; }
            if v3 < threshold { mask_l |= 8; }
            if v4 < threshold { mask_l |= 16; }
            if v7 < threshold { mask_l |= 128; }

            for x in 0..grid.width - 1 {
                let base_idx = zy_base + x;
                let x_pos = grid.origin.x + (x as f32) * grid.voxel_size.x;

                // Load "Right Face" values (vertices 1, 2, 5, 6)
                // These are located at x+1 relative to the base (current x).
                // Safety: Max access is x+1+stride_y+stride_z, which is within bounds
                // as checked in the preamble.
                let next_base_idx = base_idx + 1;
                let v1 = unsafe { *data.get_unchecked(next_base_idx) };
                let v2 = unsafe { *data.get_unchecked(next_base_idx + stride_y) };
                let v5 = unsafe { *data.get_unchecked(next_base_idx + stride_z) };
                let v6 = unsafe { *data.get_unchecked(next_base_idx + stride_y + stride_z) };

                // 1. Determine the index of the case (0-255)
                let mut mask_r = 0;
                if v1 < threshold { mask_r |= 2; }
                if v2 < threshold { mask_r |= 4; }
                if v5 < threshold { mask_r |= 32; }
                if v6 < threshold { mask_r |= 64; }

                let cube_index = mask_l | mask_r;
                ...
                if edge_flags != 0 {
                    ...
                    if let Some(grads) = cached_gradients {
                        ...
                        corner_normals[0] =
                            get_gradient_interior(data, base_idx, stride_y, stride_z);
                        corner_normals[3] =
                    ...
                    if can_use_fast_path {
                        ...
                        corner_normals[1] =
                            get_gradient_interior(data, next_x_idx, stride_y, stride_z);
                        corner_normals[2] =
                ...
                // Update sliding window: Right Face becomes Left Face for next iteration
                v0 = v1;
                v3 = v2;
                v4 = v5;
                v7 = v6;

                // Update mask: Map Right Face bits to Left Face bits
                // 2 (v1) -> 1 (v0)
                // 4 (v2) -> 8 (v3)
                // 32 (v5) -> 16 (v4)
                // 64 (v6) -> 128 (v7)
