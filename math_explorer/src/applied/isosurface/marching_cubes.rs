...
#[inline]
fn get_gradient<V: Volume + ?Sized>(volume: &V, x: usize, y: usize, z: usize) -> Point3D {
    let (width, height, depth) = volume.dimensions();

    let dx = if x == 0 {
        volume.get(x + 1, y, z) - volume.get(x, y, z)
    } else if x == width - 1 {
        volume.get(x, y, z) - volume.get(x - 1, y, z)
    } else {
        (volume.get(x + 1, y, z) - volume.get(x - 1, y, z)) / 2.0
    };

    let dy = if y == 0 {
        volume.get(x, y + 1, z) - volume.get(x, y, z)
    } else if y == height - 1 {
        volume.get(x, y, z) - volume.get(x, y - 1, z)
    } else {
        (volume.get(x, y + 1, z) - volume.get(x, y - 1, z)) / 2.0
    };

    let dz = if z == 0 {
        volume.get(x, y, z + 1) - volume.get(x, y, z)
    } else if z == depth - 1 {
        volume.get(x, y, z) - volume.get(x, y, z - 1)
    } else {
        (volume.get(x, y, z + 1) - volume.get(x, y, z - 1)) / 2.0
    };
...
pub fn extract_isosurface<V: Volume + ?Sized>(volume: &V, threshold: f32) -> Result<Mesh, String> {
    let (width, height, depth) = volume.dimensions();
    if width < 2 || height < 2 || depth < 2 {
        return Err("Volume dimensions must be at least 2x2x2".to_string());
    }
    ...
    for z in 0..depth - 1 {
        ...
        for y in 0..height - 1 {
            ...
            for x in 0..width - 1 {
                ...
                // Direct access for corner values
                // Vertices are ordered:
                // 0: (0,0,0), 1: (1,0,0), 2: (1,1,0), 3: (0,1,0)
                // 4: (0,0,1), 5: (1,0,1), 6: (1,1,1), 7: (0,1,1)

                let v0 = volume.get(x, y, z);
                let v1 = volume.get(x + 1, y, z);
                let v2 = volume.get(x + 1, y + 1, z);
                let v3 = volume.get(x, y + 1, z);
                let v4 = volume.get(x, y, z + 1);
                let v5 = volume.get(x + 1, y, z + 1);
                let v6 = volume.get(x + 1, y + 1, z + 1);
                let v7 = volume.get(x, y + 1, z + 1);
                ...
                if let Some(grads) = cached_gradients {
                    ...
                    corner_normals[0] = get_gradient(volume, x, y, z);
                    corner_normals[3] = get_gradient(volume, x, y + 1, z);
                    corner_normals[4] = get_gradient(volume, x, y, z + 1);
                    corner_normals[7] = get_gradient(volume, x, y + 1, z + 1);
                }
                ...
                corner_normals[1] = get_gradient(volume, x + 1, y, z);
                corner_normals[2] = get_gradient(volume, x + 1, y + 1, z);
                corner_normals[5] = get_gradient(volume, x + 1, y, z + 1);
                corner_normals[6] = get_gradient(volume, x + 1, y + 1, z + 1);
