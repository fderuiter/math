...
/// A calculator for determining the favoritism score using a provided RNG strategy.
pub struct FavoritismCalculator<R: Rng> {
    rng: R,
}

impl<R: Rng> FavoritismCalculator<R> {
    /// Creates a new `FavoritismCalculator` with the given random number generator.
    pub fn new(rng: R) -> Self {
        Self { rng }
    }

    /// Calculates the favoritism score based on the provided inputs.
    pub fn calculate(&mut self, inputs: &FavoritismInputs) -> f64 {
        ...
        let r = self.rng.gen_range(0.9..1.1);
...
#[deprecated(note = "Use FavoritismCalculator::new(rng).calculate(inputs) instead to allow dependency injection.")]
pub fn calculate_favoritism_score(inputs: &FavoritismInputs) -> f64 {
    let mut calculator = FavoritismCalculator::new(rand::thread_rng());
