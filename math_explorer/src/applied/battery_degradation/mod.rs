...
//! - [`error`]: Errors related to battery degradation.
...
pub fn n70(d: f64) -> f64 {
    ...
    PowerLawModel::standard()
        .n70(DepthOfDischarge::new(d_clamped).expect("Clamped value should be valid"))
        .expect("Standard model calculation failed")
        .as_f64()
}
...
pub fn capacity(n: f64, d: f64) -> f64 {
    ...
    PowerLawModel::standard()
        .capacity(
            Cycles::new(n_clamped).expect("Clamped value should be valid"),
            DepthOfDischarge::new(d_clamped).expect("Clamped value should be valid"),
        )
        .expect("Standard model calculation failed")
        .as_f64()
}
...
pub fn cycles_to_capacity(target_capacity: f64, d: f64) -> f64 {
    ...
    PowerLawModel::standard()
        .cycles_to_capacity(
            Capacity::new(target_clamped).expect("Clamped value should be valid"),
            DepthOfDischarge::new(d_clamped).expect("Clamped value should be valid"),
        )
        .expect("Standard model calculation failed")
        .as_f64()
