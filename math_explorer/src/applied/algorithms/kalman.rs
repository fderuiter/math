...
impl<TFloat: RealField + Copy, T: KalmanModel<TFloat>> KalmanSystem<TFloat> for T {
    fn predict_state(&self, state: &DVector<TFloat>, dt: TFloat) -> (DVector<TFloat>, DMatrix<TFloat>) {
        let f = self.transition_matrix(dt);
        let x_pred = &f * state;
        (x_pred, f)
    }

    fn predict_measurement(&self, state: &DVector<TFloat>) -> (DVector<TFloat>, DMatrix<TFloat>) {
        let h = self.measurement_matrix();
        let z_pred = &h * state;
        (z_pred, h)
    }

    fn process_noise(&self, dt: TFloat) -> DMatrix<TFloat> {
        KalmanModel::process_noise(self, dt)
    }

    fn measurement_noise(&self) -> DMatrix<TFloat> {
