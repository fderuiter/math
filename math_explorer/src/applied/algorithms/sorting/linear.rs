...
impl Sorter<u64> for RadixSort {
    fn sort(&self, data: &mut [u64]) -> SortingStats {
        ...
        stats.comparisons += data.len() as u64 - 1;

        let mut exp = 1;
        while max_val / exp > 0 {
            counting_sort_for_radix(data, exp, &mut stats);
            exp *= 10;
        }
        stats
    }
...
#[deprecated(since = "0.2.0", note = "Use `RadixSort` struct with `Sorter` trait instead.")]
pub fn radix_sort(data: &[u64]) -> SortingResult<u64> {
    let mut sorted_data = data.to_vec();
    let stats = RadixSort.sort(&mut sorted_data);
