...
pub struct HodgkinHuxleyParameters {
    ...
    pub v_rest: f64,
    /// Gating kinetics for Potassium activation ($n$).
    pub n_kinetics: Arc<dyn GatingKinetics>,
    /// Gating kinetics for Sodium activation ($m$).
    pub m_kinetics: Arc<dyn GatingKinetics>,
    /// Gating kinetics for Sodium inactivation ($h$).
...
impl Default for HodgkinHuxleyParameters {
    fn default() -> Self {
        ...
        Self {
            ...
            e_l: v_rest + 10.6,
            v_rest,
            n_kinetics: Arc::new(StandardPotassiumKinetics),
            m_kinetics: Arc::new(StandardSodiumActivationKinetics),
            h_kinetics: Arc::new(StandardSodiumInactivationKinetics),
        }
    }
}
