...
/// A pure physics model for Reaction-Diffusion.
///
/// Separates the immutable laws (Reaction + Diffusion) from the mutable simulation state.
...
pub struct ReactionDiffusionSystem<R: ReactionModel, D: DiffusionModel> {
    ...
    /// The immutable physics model.
    pub model: ReactionDiffusionModel<R, D>,
    /// Default solver used for `step()`.
    default_solver: Euler<ChemicalState>,
}
...
impl<R: ReactionModel, D: DiffusionModel> ReactionDiffusionSystem<R, D> {
    pub fn new(
        ...
        // Create Euler solver. It requires an example state to allocate its internal buffer.
        let default_solver = Euler::new(&state);
    ...
    pub fn step(&mut self, dt: f64) {
        // Delegate to the solver, passing the immutable model (OdeSystem) and mutable state.
        // This avoids borrow checker conflicts because `model` and `state` are disjoint fields.
        self.default_solver
            .step(&self.model, 0.0, &mut self.state, dt);
    }

    /// Advances the system using a custom solver.
    pub fn step_with<S: Solver<ChemicalState>>(&mut self, solver: &mut S, dt: f64) {
        solver.step(&self.model, 0.0, &mut self.state, dt);
    }
}
