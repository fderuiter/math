...
pub trait SpatialDiffusion<const N: usize> {
    ...
    /// * `vals`: Current values of species at index (array of size N).
    /// * `diffs`: The diffusion terms for species ($D_i \nabla^2 u_i$) (array of size N).
    ...
    ///
    /// Computes $D_i \nabla^2 u_i$ and stores the result in `out`.
    ///
    /// # Arguments
...
mod tests_2d {
    ...
    fn test_laplacian_uniform() {
        ...
        // Apply for 2 components
        let state = [&u[..], &v[..]];
        let mut out = [&mut out_u[..], &mut out_v[..]];
        let coeffs = [1.0, 1.0];

        diff.apply(state, &mut out, coeffs);
    ...
    fn test_map_diffusion_equivalence() {
        ...
        {
            let mut out_1 = [&mut out_u_1[..], &mut out_v_1[..]];
            diff.apply(state, &mut out_1, coeffs);
        }
        ...
        diff.map_diffusion(state, coeffs, |i, vals, diffs| {
            let reac = [1.0, 2.0];
            out_u_2[i] = vals[0] + dt * (diffs[0] + reac[0]);
            out_v_2[i] = vals[1] + dt * (diffs[1] + reac[1]);
        });
