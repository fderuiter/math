...
pub struct TuringSystem<
    ...
    S: TuringSolverStrategy<K, D> = FusedEulerSolver,
> {
...
impl TuringSystem<SchnakenbergKinetics, FiniteDifference1D, FusedEulerSolver> {
    /// Creates a new Turing System with default strategies.
...
impl<K: ReactionKinetics, D: SpatialDiffusion<2>> TuringSystem<K, D, FusedEulerSolver> {
    ...
    /// Uses the default FusedEulerSolver.
...
impl<K: ReactionKinetics, D: SpatialDiffusion<2>, S: TuringSolverStrategy<K, D>>
    TuringSystem<K, D, S>
{
    /// Creates a new Turing System with full custom configuration.
    ...
    pub fn step(&mut self, dt: f64) {
        // Delegate to the solver strategy
        self.solver.step(
            &self.state,
            &mut self.next_state,
            &self.kinetics,
...
impl<K: ReactionKinetics, D: SpatialDiffusion<2>, S: TuringSolverStrategy<K, D>>
    OdeSystem<TuringState> for TuringSystem<K, D, S>
{
...
impl<K: ReactionKinetics, D: SpatialDiffusion<2>, S: TuringSolverStrategy<K, D>>
    TimeStepper<TuringState> for TuringSystem<K, D, S>
{
    ...
    fn step(&mut self, dt: f64) {
        // Delegate to the optimized inherent method which uses the solver
        self.step(dt);
    }
}
