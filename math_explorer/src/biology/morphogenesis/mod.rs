...
impl<K: ReactionKinetics, D: SpatialDiffusion<2>, S: TuringSolverStrategy> TuringSystem<K, D, S> {
    ...
    ///
    /// This method delegates the integration step to the configured `solver`.
    pub fn step(&mut self, dt: f64) {
        // Delegate to the solver strategy
        // We pass components individually to avoid borrowing `self` entirely
        // while needing to mutate `self.state` and `self.next_state`.
...
mod tests {
    ...
    fn test_turing_system_logic_preservation() {
        ...
        // Expected values captured from baseline run (Forward Euler)
        let expected_u = vec![
            0.9798926377401955,
            1.0722504645444493,
            1.1685990805783317,
