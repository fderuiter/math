...
impl InteractiveTool for PrimeSpiralWidget {
    ...
    fn show(&mut self, ctx: &egui::Context) {
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.vertical(|ui| {
                ui.horizontal(|ui| {
                    if ui.add(egui::Slider::new(&mut self.grid_size, 10..=math_commons::registry::MAX_GRID_SIZE).text("Grid Size (side length)"))
                        .changed()
                    {
...
impl scientific_metadata::theory::TheoryDescribable for PrimeSpiralWidget {
