...
impl GraphTheoryTool for GraphEditorTool {
    ...
    fn show(&mut self, ctx: &egui::Context) {
        egui::SidePanel::left("graph_editor_controls").show(ctx, |ui| {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            for (i, &(u, v, weight)) in self.edges.iter().enumerate() {
                if let (Some(&pos_u), Some(&pos_v)) =
                    (self.node_positions.get(&u), self.node_positions.get(&v))
                {
                    let color = if self.mode == EditorMode::Remove && hovered_edge == Some(i) {
                        egui::Color32::RED
                    } else {
                        egui::Color32::GRAY
                    };
            ...
            for (&id, &pos) in &self.node_positions {
                ...
                let is_hovered = hovered_node == Some(id);
            ...
            // Clicking logic
            if response.clicked() && self.dragged_node.is_none() {
                if let Some(pos) = pointer_pos {
                    match self.mode {
                        EditorMode::AddNode => {
                            if hovered_node.is_none() {
                        ...
                        EditorMode::AddEdge => {
                            if let Some(clicked_id) = hovered_node {
                        ...
                        EditorMode::Remove => {
                            if let Some(clicked_id) = hovered_node {
                                ...
                                self.rebuild_graph();
                            } else if let Some(edge_idx) = hovered_edge {
                                // Remove Edge
