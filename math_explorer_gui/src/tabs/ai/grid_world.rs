    ...
    fn show(&mut self, ctx: &egui::Context) {
        egui::Window::new("Grid World Navigation (Q-Learning)").show(ctx, |ui| {
            ui.horizontal(|ui| {
                if ui
                    .button("▶ Step")
                    .on_hover_text("Advance the agent by one single action")
                    .clicked()
                {
                    self.step_agent();
                }
                if ui
                    ...
                    .on_hover_text("Run the agent continuously for 100 episodes to update Q-values")
                    .clicked()
                {
                    for _ in 0..100 {
                ...
                if ui
                    .button("↻ Reset Agent")
                    .on_hover_text("Clear all learned Q-values and restart from episode 0")
                    .clicked()
                {
