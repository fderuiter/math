...
impl AiTool for LossLandscapeTool {
    ...
    fn show(&mut self, ctx: &egui::Context) {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            let mut loss_grid = Vec::with_capacity(self.resolution * self.resolution);
            ...
            for i in 0..self.resolution {
                for j in 0..self.resolution {
                    ...
                    // ⚡ Bolt Optimization:
                    // Avoid allocating a new DVector for predictions inside this nested loop
                    // (which runs on every frame). Instead, calculate the MSE manually using scalar operations.
                    ...
                    let loss = if !self.x_data.is_empty() {
                        sum_squared_error / self.x_data.len() as f64
                    } else {
                        0.0
                    };
