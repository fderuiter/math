...
fn main() {
    ...
    // This parses Cargo.toml for path dependencies and scans their src/ directories
    let cargo_toml = fs::read_to_string("Cargo.toml").unwrap_or_default();
    for line in cargo_toml.lines() {
        if line.contains("path = ") {
            // Very basic parse of `name = { path = "../path" }`
            ...
            if let Some(start) = line.find("path = \"") {
                ...
                if let Some(end) = rest.find('"') {
                    ...
                    if abs_dep_path.exists() {
                        scan_external_crate(&abs_dep_path, &crate_name, &mut discovered_tabs, &abs_dep_path);
                    }
                }
            }
        }
    ...
    generated_code.push_str("\npub fn instantiate_tabs() -> Vec<Box<dyn crate::tabs::ExplorerTab>> {\n");
                    ...
                    if let Some(s_name) = extract_struct_name(&content) {
                        ...
                        if let Ok(rel_path) = path.strip_prefix(root_src) {
                            ...
                            for comp in rel_path.components() {
                                ...
                                if c.ends_with(".rs") {
                                    comps.push(c[..c.len()-3].to_string());
                                } else {
                        ...
                        let instantiation = format!("{}{}{}::default()", crate_name, mod_path, if mod_path.is_empty() { "::" } else { "::" });
                        
                        // Actually, a simpler way is just crate_name::mod_path::StructName
                        // Let's refine it:
                        let clean_crate_name = crate_name.replace("-", "_");
                        let full_path = format!("{}{}{}", clean_crate_name, mod_path, if mod_path.is_empty() { "::" } else { "::" });
