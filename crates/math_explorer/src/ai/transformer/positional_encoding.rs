...
mod tests {
    ...
    fn test_positional_encoding_values_at_pos_0() {
        ...
        for i in 0..(512 / 2) {
            // sin(0) = 0
            assert_relative_eq!(pe[(0, 2 * i)], 0.0, epsilon = 1e-9);
            // cos(0) = 1
            assert_relative_eq!(pe[(0, 2 * i + 1)], 1.0, epsilon = 1e-9);
        }
    }
    ...
    fn test_positional_encoding_arbitrary_value() {
        ...
        assert_relative_eq!(pe[(pos, 2 * i)], angle.sin(), epsilon = 1e-9);
        assert_relative_eq!(pe[(pos, 2 * i + 1)], angle.cos(), epsilon = 1e-9);
    }
}
