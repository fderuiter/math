...
mod tests {
    ...
    fn test_hu_to_density() {
        // Water
        assert!((hu_to_density(0.0).unwrap().value() - 1.0).abs() < 1e-6);
        // Air
        assert!((hu_to_density(-1000.0).unwrap().value() - 0.0).abs() < 1e-6);
        // Bone
        // HU = 500 -> rho = 1 + 500/500 = 2.0
        assert!((hu_to_density(500.0).unwrap().value() - 2.0).abs() < 1e-6);
