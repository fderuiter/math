...
mod tests {
    ...
    fn test_z_consistency() {
        ...
        assert!(
            (z - 1.0).abs() < math_commons::registry::TOLERANCE_STANDARD,
            "Z should be 1 for single E=0 state"
        );
        assert!(
            f.abs() < math_commons::registry::TOLERANCE_STANDARD,
            "F should be 0 for Z=1"
        );
    }
}
