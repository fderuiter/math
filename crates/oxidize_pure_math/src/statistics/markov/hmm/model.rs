...
///
/// ```rust
/// use oxidize_pure_math::statistics::markov::hmm::HiddenMarkovModel;
...
impl<T: RealField + Copy + ToPrimitive> HiddenMarkovModel<T> {
    ...
    pub fn new(
        ...
        crate::statistics::markov::validation::validate_probability_vector(&initial)?;

        // Validate transition matrix
        crate::statistics::markov::validation::validate_stochastic_matrix(&transitions)?;

        // Validate emission matrix
        crate::statistics::markov::validation::validate_stochastic_matrix(&emissions)?;
