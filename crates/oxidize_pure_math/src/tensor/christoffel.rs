...
///
/// ```
/// use oxidize_pure_math::tensor::christoffel::christoffel_symbols;
/// use oxidize_pure_math::tensor::metric::RiemannianMetric;
/// use nalgebra::{DMatrix, DVector};
///
...
pub fn christoffel_symbols(
    ...
    for k in 0..dim {
        // Forward shift
        p_eval[k] += h;
        let g_plus = metric.metric_at(&p_eval)?;

        // Backward shift
        p_eval[k] -= 2.0 * h;
        let g_minus = metric.metric_at(&p_eval)?;
