...
pub fn generate_noise(rows: usize, cols: usize) -> Result<DMatrix<f64>, crate::AIError> {
    let mut rng = rand::thread_rng();
...
pub fn generate_noise_with_rng<R: Rng + ?Sized>(
    ...
    cols: usize,
    rng: &mut R,
) -> Result<DMatrix<f64>, crate::AIError> {
    let normal = Normal::new(0.0, 1.0).map_err(|e| crate::AIError::ConversionError {
