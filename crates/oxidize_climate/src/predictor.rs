...
use oxidize_ai::optimization::Optimizer;
use crate::autoencoder::{ConvLayer, leaky_relu};
...
pub trait PredictorModel {
    ...
    fn update_weights<O: Optimizer<f32>>(
        &mut self,
        optimizer: &mut O,
    ) -> Result<(), oxidize_ai::optimization::OptimizationError>;
}
...
impl PredictorModel for Predictor {
    fn forward(&self, input: &DMatrix<f32>) -> DMatrix<f32> {
        ...
        for (i, layer) in self.layers.iter().enumerate() {
            ...
            x = crate::tensor_ops::conv1d(&x, &layer.kernel, &layer.bias);
...
mod tests {
    use super::*;
    use oxidize_ai::optimization::SGD;
