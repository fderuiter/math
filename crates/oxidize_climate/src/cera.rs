...
use crate::autoencoder::{Autoencoder, AutoencoderModel};
use crate::predictor::{Predictor, PredictorModel};
use nalgebra::DMatrix;
// Re-export CeraConfig for backward compatibility (or convenience)
pub use crate::config::CeraConfig;
...
mod tests {
    use super::*;
    use crate::training::CeraTrainer;
    ...
    fn test_cera_with_custom_models() {
        ...
        let encoder_layers = vec![
            crate::autoencoder::ConvLayer::new(config.in_channels, 32),
            crate::autoencoder::ConvLayer::new(32, config.latent_channels),
        ];
        let encoder = crate::autoencoder::Encoder::new_from_layers(encoder_layers);
        let decoder =
            crate::autoencoder::Decoder::new(config.latent_channels, config.in_channels);
        let autoencoder =
            crate::autoencoder::Autoencoder::new_from_components(encoder, decoder);
