...
## 2027-02-15 - Dependency Inversion for Ising Simulation
**Violation:** The `SpinLattice` struct hardcoded `rand::thread_rng()` and mixed state storage with simulation logic (SRP violation), making deterministic testing impossible.
**Refactor:** Applied **Dependency Inversion** and **Strategy Pattern** (conceptually). Extracted `MetropolisSolver` struct with injected RNG (`R: Rng`).
**Trade-off:** Minimal boilerplate for solver instantiation, but enabled deterministic testing and separation of concerns (Physics vs Simulation).
