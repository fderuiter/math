...
We implement these concepts in Rust, creating a new module within the \texttt{math_explorer} project. This implementation relies on the \texttt{nalgebra} library for its linear algebra capabilities \cite{nalgebra}.

\subsection{Combining LoRA Modules}

The \texttt{combine_loras} function takes a slice of LoRA state dictionaries and a slice of weights and returns their weighted sum. A \texttt{LoraStateDict} is represented as a \texttt{HashMap<String, DMatrix<f64>>}.
...
The objective function is implemented by \texttt{calculate_objective_score}, which combines a (mock) loss value with the L1 regularization term.
